...
export default function TintShadeGenerator({ typeOfVariation }) {
  ...
  try {
    tints = new Values(`${colorDetails.color}`).tints(
      colorDetails.tintShadePercent
    );
    shades = new Values(`${colorDetails.color}`).shades(
      colorDetails.tintShadePercent
    );
  } catch (error) {
  ...
  const pressToCopy = (e, color)=> {
    if (e.key === 'Enter' || e.key === ' ') {
      copyToClipBoardHandler(color);
    }
  }
    ...
    <div className="mb-4 md:w-[90vw] flex flex-col justify-center w-full">
      ...
      <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        {currentVariation.map((variation) => (
          <div key={`${variation.hex}${Math.random() * 100}`}>
            {`#${variation.hex}` === colorDetails.currentShadeOrTint ? (
              <span
                className={`${
                  variationShade ? "bg-white text-black" : "bg-black text-white"
                } w-20 absolute mt-6 md:ml-7 ml-10 text-center text-sm`}
              >
                copied
              </span>
            ) : (
            ...
            <li
              className={`bg-gray-200  text-center h-64 ${
                variationShade ? "text-white" : "text-black"
              }  text-[17px] flex flex-col gap-5 items-center justify-center p-8`}
              style={{
            ...
            >
                ...
                <div className="flex flex-row flex-nowrap gap-4 text-2xl cursor-pointer">
                  {`#${variation.hex}` === colorDetails.currentShadeOrTint ? (
                    ...
                    <div className="flex justify-center items-center rounded-full hover:bg-black hover:bg-opacity-10 w-10 h-10">
                      <LuCopy
                        onClick={() =>
                          copyToClipBoardHandler(`#${variation.hex}`)
                        }
                      />
                    </div>
                  )}
                  <div className="flex justify-center items-center rounded-full hover:bg-black hover:bg-opacity-10 w-10 h-10">
                    <HiOutlineEyeDropper
                      onClick={() => colorChoice(variation.hex)}
                    />
                  </div>
                </div>
              }
              ...
              <p
                className="cursor-pointer hover:bg-black hover:bg-opacity-10 w-20"
                onClick={() => copyToClipBoardHandler(`#${variation.hex}`)}
                onKeyDown = {(e)=>pressToCopy(e, `#${variation.hex}`)}
                tabIndex = {0}
              >{`#${variation.hex.toUpperCase()}`}</p>
            </li>
          </div>
        ))}
      </ul>
...
TintShadeGenerator.propTypes = {
  typeOfVariation: PropTypes.oneOf(["SHADES", "TINTS"]).isRequired,
};
